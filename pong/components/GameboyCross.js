class GameboyCross extends HTMLElement {
  ...
  addListeners() {
    ...
    const startPress = e => {
      if (e.target.classList.contains('button-top')) {
        if (!this.interval) {
          this.interval = setInterval(() => {
            this.dispatchEvent(
              new CustomEvent('pong-paddel-to-top', {
                bubbles: true,
                composed: true
              })
            );
      ...
      } else if (e.target.classList.contains('button-bottom')) {
        if (!this.interval) {
          this.interval = setInterval(() => {
            this.dispatchEvent(
              new CustomEvent('pong-paddel-to-bottom', {
                bubbles: true,
                composed: true
              })
            );
    ...
    const stopPress = () => {
      ...
      this.dispatchEvent(
        new CustomEvent('pong-paddel-to-bottom-mouseup', {
          bubbles: true,
          composed: true
        })
      );
      this.dispatchEvent(
        new CustomEvent('pong-paddel-to-top-mouseup', {
          bubbles: true,
          composed: true
        })
      );
    ...
    parentComponent.addEventListener('click', e => {
      if (e.target.matches('.button-top')) {
        this.dispatchEvent(
          new CustomEvent('pong-paddel-to-top-click', {
            bubbles: true,
            composed: true
          })
        );
        ...
        this.dispatchEvent(
          new CustomEvent('pong-paddel-to-bottom-click', {
            bubbles: true,
            composed: true
          })
        );
