...
class GameboyConsole extends HTMLElement {
  ...
  static get styles() {
    return /* css */ `
      ...
      .checkbox-music {
        --sound-yes: url('intro/assets/sound-yes.svg');
        --sound-no: url('intro/assets/sound-no.svg');
        display: inline-block;
        margin-left: 10px;
        translate: 0 7px;
        outline: 0;
  ...
  connectedCallback() {
    ...
    this.shadowRoot
      ...
      .addEventListener('click', e => {
        ...
        const gameboyCrystal = this.shadowRoot.querySelector('gameboy-crystal');
        const isOn = powerSwitch.classList.contains('on');
        gameboyCrystal.setAttribute('power', isOn);
        if (!isOn) {
          location.reload();
        }
      });
